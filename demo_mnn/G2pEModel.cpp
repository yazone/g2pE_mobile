...
G2pEModel::G2pEModel(const char* modelFilename)
{
...
G2pEModel::~G2pEModel()
{
    ...
    Interpreter::destroy(interpreter_encoder);
    Interpreter::destroy(interpreter_decoder);
};

void G2pEModel::interpreterBuild(const char* modelFilename)
{
    ...
    /*创建encoder模型*/
    interpreter_encoder = Interpreter::createFromFile("models/g2pE_mobile_encoder.mnn");
    session_encoder = interpreter_encoder->createSession(config, runtimeInfo);
    
    /*创建decoder模型*/
    interpreter_decoder = Interpreter::createFromFile("models/g2pE_mobile_decoder.mnn");
